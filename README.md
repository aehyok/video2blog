## 第二个版本
- 将首页只作为一个展示列表，上面是一个搜索输入框，下载完毕，就展示在下面列表中
- 点击列表中的项，便可以跳转到详情页面，详情页面就没有左侧的视频列表，直接就是转录的字幕了，然后可以右键进行翻译和生成文章
- 英文可以翻译为中文
- 文字转录为音频
- 获取图片可以加载全部 通过/ 进行加载区分
- 后期可以考虑，对局部文字段落进行重新翻译，或者重新生成内容
- prompt要有地方进行设置
- 获取图片的时候进行设置阈值  调整图片相似度
- whisper模型设置
